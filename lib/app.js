...
app.get('/api/my_garden/:id', async (req, res) => {
  try {
    const data = await client.query(
      `SELECT * from my_garden 
      JOIN notes 
      ON my_garden.user_id = notes.user_id 
      AND my_garden.main_species_id = notes.main_species_id`
    );
...
app.get('/api/my_garden', async (req, res) => {
  try {
...
app.post('/api/notes', async (req, res) => {
  try {
    const data = await client.query(
      `INSERT INTO notes 
      (main_species_id, user_id, date, note) 
      VALUES ($1,$2,$3,$4) 
      RETURNING *`,
...
app.get('/api/plant_detail/:id', async (req, res) => {
  try {
    const data = await request.get(`https://trefle.io/api/v1/plants/${req.params.id}`)
      .set('Authorization', `${process.env.TREFLE_TOKEN}`);
...
app.get('/api/filtered_search', async (req, res) => {
  try {
    const data = await request.get(`https://trefle.io/api/v1/plants?filter[edible_part]=${req.query.part}&filter[vegetable]=${req.query.vegetable}&range[light]=${req.query.light}`)
      .set('Authorization', `${process.env.TREFLE_TOKEN}`);
...
app.get('/api/name_search', async (req, res) => {
  try {
    const data = await request.get(`https://trefle.io/api/v1/plants/search?q=${req.query.name}`)
      .set('Authorization', `${process.env.TREFLE_TOKEN}`);
